// DRÖMGÅRDEN — co-op över PeerJS (host-auktoritativ). Samma beprövade lager som VILDMARKEN 2.
const PREFIX = 'dromgarden-';
...
export class Net {
  ...
  host(onReady) {
    ...
    const tryCode = () => {
      ...
      peer.on('error', (err) => {
        ...
        else { console.warn('peer error', err.type); this._emit('netError', err.type); }
      });
    };
    tryCode();
  }
  ...
  _setupHostConn(conn) {
    conn.on('open', () => {
      ...
      conn.on('close', () => { this.conns.delete(conn.peer); this._emit('peerLeave', conn.peer); });
  ...
  join(code, onReady, onFail) {
    ...
    peer.on('error', (err) => {
      if (!opened && (err.type === 'peer-unavailable' || err.type === 'network' || err.type === 'server-error')) onFail(err.type);
    });
  }

  send(t, d) { if (this.hostConn?.open) this.hostConn.send({ t, d }); }
  broadcast(t, d, except) { for (const [id, c] of this.conns) if (id !== except && c.open) c.send({ t, d }); }
  sendTo(peerId, t, d) { const c = this.conns.get(peerId); if (c?.open) c.send({ t, d }); }
